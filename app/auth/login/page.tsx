...
export default function LoginPage() {
  ...
  const handleGoogle = async () => {
    ...
    if (error) {
      ...
      setError(
        msg.includes('popup-closed') ? 'Sign-in was cancelled.' :
  ...
  return (
    <div className="space-y-7">
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground">Sign in to access your AdSense audit dashboard</p>
      </div>
      ...
      {/* Google CTA */}
      <Button
        className="w-full h-13 gap-3 text-[15px] font-semibold shadow-lg shadow-primary/20 rounded-xl"
        style={{ height: '52px' }}
      ...
      <div className="rounded-2xl border border-border/60 bg-muted/30 p-5 space-y-3">
        <p className="text-[11px] font-bold text-muted-foreground uppercase tracking-widest">What you get</p>
        {[
          'AdSense readiness score (0–100)',
          'AI-powered content & policy analysis',
      ...
      <div className="relative">
        ...
        <div className="relative flex justify-center">
          <span className="bg-background px-3 text-xs text-muted-foreground">New here?</span>
        </div>
      </div>
